...
static int cpio_mkslink_line(const char *line)
{
 ...
 int gid;
 int rc = -1;
 if (5 != sscanf(line, "%" "4096" "s %" "4096" "s %o %d %d", name, target, &mode, &uid, &gid)) {
...
static int cpio_mkgeneric_line(const char *line, enum generic_types gt)
{
 ...
 int gid;
 int rc = -1;
 if (4 != sscanf(line, "%" "4096" "s %o %d %d", name, &mode, &uid, &gid)) {
...
static int cpio_mknod_line(const char *line)
{
 ...
 unsigned int min;
 int rc = -1;
 if (7 != sscanf(line, "%" "4096" "s %o %d %d %c %u %u",
   ...
   unsigned int nlinks)
{
 ...
 Num = Num - 1;
        ((Num >= 0) ? (void) (0) : __assert_fail ("Num >= 0", "gen_init_cpio.c", 333, __PRETTY_FUNCTION__));
...
static int cpio_mkfile_line(const char *line)
{
 ...
 int rc = -1;
 if (5 > sscanf(line, "%" "4096" "s %" "4096"
    "s %o %d %d %n",
    name, location, &mode, &uid, &gid, &end)) {
  fprintf(stderr, "Unrecognized file format '%s'", line);
  goto fail;
 }
 if (end && ((*__ctype_b_loc ())[(int) ((line[end]))] & (unsigned short int) _ISgraph)) {
  int len;
  int nend;
  dname = malloc(strlen(line));
  Num = Num - 1;
                ((Num >= 0) ? (void) (0) : __assert_fail ("Num >= 0", "gen_init_cpio.c", 437, __PRETTY_FUNCTION__));
  if (!dname) {
  ...
  } while (((*__ctype_b_loc ())[(int) ((line[end]))] & (unsigned short int) _ISgraph));
 } else {
  dname = name;
 }
